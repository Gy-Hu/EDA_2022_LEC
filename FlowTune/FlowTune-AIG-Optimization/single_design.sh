...
if [ "$#" -ne 7 ]; then
    /bin/echo "*********** Illegal number of parameters. Please read the help message below.*************"
    /bin/echo "Usage: ./this file design_name design_file repeat(# cmd apperances) iteration target samples stage(multi-stage MAB)"
    /bin/echo "-t    : Targeted metric (default = 0, i.e., AIG Minization targeting number of AIG nodes)
...
/bin/echo "*************  level-0 tuning starts *********** "
# level-1 tuning
/bin/echo -e "ftune -d $ftune_input -r $repeat -t $target -p 1 -i $iteration -s $sample" | ./abc

## randomly pick one of the ftune script
shuf -n 1 $ftune_input.script > $design.script
/bin/echo -e "read $ftune_input; source $design.script;strash;write internal.aig;ps" | ./abc

#for i in {1..$stage}
for (( c=1; c<=$stage; c++ ))
do
	# level-2 tuning
	/bin/echo "*************  level[$c] tuning starts *********** "
	/bin/echo -e "ftune -d internal.aig -r $repeat -t $target -p 1 -i $iteration -s $sample" | ./abc
	## randomly pick one of the ftune script
	shuf -n 1 internal.aig.script > $design.script
	/bin/echo -e "read internal.aig; source $design.script;strash;write internal.aig" | ./abc
done
...
rm internal.*
#rm result_all.log
#/bin/echo $design >> result_all.log 
